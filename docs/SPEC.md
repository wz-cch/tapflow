...
**摺疊態** — 56dp 圓球,顏色示意狀態(灰 待機 / 紅 錄製 / 綠 播放 / 黃 暫停),球面顯示當下唯一有意義的動作(`▶ 繼續` 或 `⊙ 繼續錄製`)。

除了手動按 `∧`,進入暫停態時會**自動**摺疊並移到螢幕上半部,把鍵盤區讓出來 —— 見 §10.3,這是人工介入能不能真的用的關鍵。

**把手態** — 按 `✕` 後,邊緣留一條 6dp 寬、半透明的細條,點一下彈回展開態。完全不擋東西,也不用回主 app 才叫得回來。
...
**自動草稿**:工作區每次變動就寫入 `filesDir/workspace.json`,服務重啟後自動還原。成本極低,避免辛苦錄的東西沒了。有未儲存變更時,標題列的 `💾` 顯示紅點;載入其他片段前會警告。
...
|---|---|
| 點一下標記 | 選取,彈出參數卡 |
| 長按 120ms 後拖曳 | 移動座標(120ms 門檻避免誤觸) |
| 拖曳滑動的起點 / 終點圓 | 只改該端點 |
| 拖曳滑動的曲線本體 | 整條平移 |
| 點空白處 | 取消選取、收起參數卡 |
| 工具列 `＋` | 螢幕中心新增點擊點,套全域預設時長 |
