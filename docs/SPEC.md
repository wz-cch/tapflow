...
**按鍵區可捲動。** 上表宣告 19 顆鍵,最多一次顯示 11 顆,橫向螢幕仍可能放不下,所以按鍵區包在一個高度上限夾在螢幕內的 ScrollView 裡。**拖曳把手刻意留在捲動區外面** —— 放在裡面 ScrollView 會攔走垂直拖曳,工具列就再也拖不動,等於把一個摸不到的控制換成另一個。
...
**只做 ±1,不做絕對位置。** 「移到第 N 步」討論過並砍掉:它服務的是重構腳本,而重構的前提是讀得懂腳本 —— 一百步用座標標示是讀不懂的(見 §9.9)。真實的需求是插完那一下的微調,那時候你完全知道自己剛做了什麼。
...
它通過了下一節那個判準,而其他被砍掉的功能沒有:**設定的那一刻你完全知道自己要這個東西按十下**,不需要讀得懂整份腳本。
...
`✎` 編輯模式 + 標記 hit-test 與拖曳 + `＋` 擷取手勢新增一步 + `⧉` 複製 + `−` 刪除 + 步驟設定頁(類型 / 座標 / 等待秒數 / 備註 / 時長 / 延遲 / 前後移一格 / 複製 / 刪除 / 翻頁 / 跳到第 N 步)+ 步驟列表 + 隔離 + 全域設定畫面(預設值、隨機抖動、大小 / 不透明度、亞光)。
