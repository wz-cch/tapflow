...
- **編輯模式的標記畫大 1.5 倍(只有編輯模式)。** 編輯是唯一「標記是觸控目標」的模式,其他模式它只是讀數。1× 時圓點 20dp、抓取半徑 26dp —— 你要瞄的東西比真正抓得到的範圍**小**,而瞄準是用眼睛瞄的。1.5× 是 30dp 的圓點在 52dp 的目標裡,方向才是對的。**抓取半徑刻意不跟著放大**:52dp 已經超過 48dp 的門檻,再放大只會讓相隔 46dp 的衍生標記開始互相吃掉觸控。
- **兩個抓取點誰近誰贏。** 原本是先測末端、命中就回傳,而且兩者同一個半徑 —— 於是**任何比半徑短的滑動,本體都抓不到**。`SWIPE_TRAVEL_PX` 是 24 個**原始像素**,3× 螢幕上 travel 只要 8dp 就算滑動,末端落在本體半徑裡吃掉每一次觸控,短滑動根本沒辦法整個移動。
- **末端與起點相距不到 32dp 時,只給本體一個抓取點。** 兩點比手指能分辨的距離還近,給兩個等於隨機給一個。那種滑動至少要能可靠地移動;要改形狀就用「重錄」,那顆鍵就是為此存在的。
