...
−           刪除選取的標記
拖曳標記     改座標
⚏ 設定頁     類型 / 座標 / 等待秒數 / 備註 / 時長 / 前置延遲 / 前後移一格 / 複製 / 刪除 / 翻頁 / 跳到第 N 步
...
**只做 ±1,不做絕對位置。** 「移到第 N 步」討論過並砍掉:它服務的是重構腳本,而重構的前提是讀得懂腳本 —— 一百步用座標標示是讀不懂的(見 §9.8)。真實的需求是插完那一下的微調,那時候你完全知道自己剛做了什麼。
