...
**每個模式各自一組按鍵,不把不相關的按鍵變灰。** 這條原則見 §9,但原本只套用在編輯模式 —— 錄製中曾經 12 顆全顯示、其中 5 顆是死的。

幾個決定的理由:

- **`⏸+` 不放待機** —— §10.1 定義非錄製狀態按它是「有選取就插在後面,沒有就插在尾端」,但待機時不可能有選取(選取只在編輯模式存在),所以它永遠只能附加在尾端。「插在選取後面」有價值,但那屬於編輯模式
- **`↩` 不放待機** —— 它當初的存在理由是「完整的選取刪除排在 M2」,而 M2 已經做完。前提是暫停點要選得到才能刪(§8.1),否則誤插一個就再也移不掉
- **`⚙` 不放錄製** —— 唯一會想在錄製中當場改的是逐手勢補發,而它正常就是開著的
- **`✕` 不放錄製** —— 它會關掉整個懸浮工具列,而那是停止錄製的唯一入口

**按鍵區可捲動。** 上表宣告 15 顆鍵,最多一次顯示 10 顆,橫向螢幕仍可能放不下,所以按鍵區包在一個高度上限夾在螢幕內的 ScrollView 裡。**拖曳把手刻意留在捲動區外面** —— 放在裡面 ScrollView 會攔走垂直拖曳,工具列就再也拖不動,等於把一個摸不到的控制換成另一個。
...
| **編輯**(按 `✎`) | 整螢幕攔截 | 標記 + 選取框(參數卡是另一個 window) |
| **錄製**(按 `⊙`) | 整螢幕攔截 + 逐手勢補發 | 半透明底色 + 即時新增的標記 |

亞光黑幕畫在這個 View 上,不需要額外 window。**參數卡不是** —— 它是自己的 window,理由見 §9。
...
- **參數卡是獨立的 window,不是畫在畫布上的圖。** 十來個控制項若要自己做 hit-test,geometry 的量遠超過收益,而且會跟標記的 hit-test 打結。代價是它跟其他 overlay 一樣 non-focusable,所以**備註那一列只是顯示 + 一顆按鍵**,按下去跳 `WorkspaceDialogActivity` 才輸入 —— overlay 拿焦點會搶掉下層 app 的輸入法。
...
那個節點**可以點選,但不能拖曳**。可點選是必要的:編輯模式的 `−` 要有選取才能刪,而在它變成標記之前,`↩ 復原上一步` 是唯一能移除暫停點的手段 —— 偏偏 `⏸+` 一按就停止錄製,把使用者丟進待機。不能拖曳也是必要的:它的位置是從前後鄰居推導出來的,拖了不會改變任何存下來的值,下次重繪就彈回去。
...
**填的地方在編輯模式的參數卡**:選取暫停點 → 「備註」那一列 → 跳出輸入畫面。走 Activity 而不是 overlay,因為自由文字需要輸入法,而懸浮視窗一律 `FLAG_NOT_FOCUSABLE`(§9)。

**插入時不問是刻意的。** 按 `⏸+` 的下一刻,你要的是目標 app 在眼前讓你動手做事 —— 那時候彈一個輸入框只會擋路。而且插入必須維持一按就好(見本節開頭)。在編輯模式插入時會自動選取剛建立的那一步,所以參數卡直接開在它上面。
...
`✎` 編輯模式 + 標記 hit-test 與拖曳 + `＋` 新增點擊點 + `−` 刪除 + 參數卡(時長 / 延遲 / 重指定座標)+ 全域設定畫面(預設值、隨機抖動、大小 / 不透明度、亞光)。
