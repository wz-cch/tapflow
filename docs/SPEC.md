...
- **插入一律往後,唯一的例外掛在設定頁上並且寫明「往前插」** —— 理由見 §9
...
−           刪除選取的標記
拖曳標記     改座標
⚏ 設定頁     類型 / 座標 / 等待秒數 / 備註 / 時長 / 前置延遲 / 往前插 / 翻頁 / 跳到第 N 步
...
### 9.3 插入一律往後,唯一的例外寫在按鍵上

**主選單的插入(`＋` `⧉` `⏸+` `⏱+`)一律插在選取之後。** 這是錄製生長的方向 —— 你加進腳本的每一步都落在前一步後面。選了 47,新的就是 48;沒選就落在尾端,同一條規則。

`insertBefore` 是更能表達的原語(它是唯一能表達「放到第 1 步前面」的操作),但把它放在主選單上讀起來是反的。所以它保留下來,掛在**設定頁上、寫明「往前插」** —— 一顆講清楚方向的按鍵不會被誤會。
...
- **複製、往前插、刪除三顆都在**,而且三顆都讓面板留在「這個動作留下的那一步」上:複本、新插入的那一步、以及刪除後**前一步**。刪除一開始被擋下來,正是因為「按完面板要去哪」沒有答案;答案定下來(往前一格)之後那個顧慮就沒了
- **三顆都跟主選單共用同一段程式**(`duplicateSelected()` / `deleteSelected()`)。兩個入口一種行為沒問題,兩份實作才會慢慢不一致
- **刪除排在最左邊、跟另外兩顆隔開。** 產生一步的兩顆放在一起,把摧毀的那顆擺在旁邊,就是伸手要按複製卻按到刪除
...
`✎` 編輯模式 + 標記 hit-test 與拖曳 + `＋` 擷取手勢新增一步 + `⧉` 複製 + `−` 刪除 + 步驟設定頁(類型 / 座標 / 等待秒數 / 備註 / 時長 / 延遲 / 往前插 / 翻頁 / 跳到第 N 步)+ 步驟列表 + 隔離 + 全域設定畫面(預設值、隨機抖動、大小 / 不透明度、亞光)。
