...
**按鍵區可捲動。** 上表宣告 17 顆鍵,最多一次顯示 11 顆,橫向螢幕仍可能放不下,所以按鍵區包在一個高度上限夾在螢幕內的 ScrollView 裡。**拖曳把手刻意留在捲動區外面** —— 放在裡面 ScrollView 會攔走垂直拖曳,工具列就再也拖不動,等於把一個摸不到的控制換成另一個。
...
- **步驟清單在編輯模式改成一個可捲動、可點選的獨立 window**,按 `☰` 開關。原本的決定是「編輯模式不畫清單,位置會跟參數卡重疊」,但那是在假設腳本很短的前提下 —— 100 步時用標記選取根本行不通:標記互相重疊,而**播放回報的是編號**(`⤳ 47 / 100`),沒有任何辦法把那個數字換回一個標記。
  - 清單裡有**跳到第 N 步**(複用等待秒數那個數字鍵盤)與上一步 / 下一步。播放說第 47 步壞了,你就輸入 47
  - 做成獨立 window 而不是畫在畫布上:畫上去的版本只能顯示塞得下的尾端、不能捲,而畫布裡的可捲區域會跟拖曳搶垂直手勢 —— 跟工具列把拖曳把手留在 ScrollView 外面是同一個坑
  - 它會蓋掉一條螢幕,所以**按 `☰` 收起來就回到用標記拖曳的狀態**。選取是共用的,所以收起清單時你選的還是同一步 —— 兩種入口不需要各自一套動作
  - **清單可拖曳**(抓標題上的 `47 / 100`),參數卡預設移到螢幕頂端。兩者原本相距不到一成螢幕高,實機上直接疊在一起。拖曳把手刻意不放在清單本體:那裡的垂直手勢要留給捲動
  - **「從第 N 步開始播」不在這裡,在待機的工具列上**(`▶|`)。決定一次執行從哪裡開始是**執行**的性質,不是編輯的性質。它會用當下的選取預填,而選取離開編輯模式後還留著 —— 所以剛改完第 47 步,數字已經在那裡了
- **編輯模式預設只顯示選取的那一個標記。** 原本是強制開到「全部」,理由是「看不到的東西沒辦法編輯」—— 但 100 步的腳本裡那是 100 個重疊的準心,而沒被選到的 99 個對當下要改的事沒有幫助。`👁` 在編輯模式改成這個隔離開關的切換(需要用眼睛找標記時關掉它);它原本在編輯模式**完全無效**,因為畫布會強制覆蓋掉它剛設的值。
  - **沒有選取時仍然全部顯示** —— 沒有東西可以隔離,而且藏光了就沒有標記可以點,選取要從某處來。
  - 隱藏的標記也不參與 hit-test:看不到的東西不該抓得到。
