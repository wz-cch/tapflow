...
- **有「複製」,但沒有「刪除」** —— 這條線不是喜好。複製與往前插都是**產生**一步,而面板接著就落在新的那一步上,你可以直接改它的數值;在第 5 步想要一個一樣的,關掉面板去按主選單再開回來是三個動作換一個。刪除**摧毀**的正好是面板現在顯示的那一步,按下去面板得跳到別處或自己關掉 —— 那是同一個動作長出第二種行為,正是「只放一個地方」要避免的事
- **複製兩處共用同一段程式。** 兩個入口一種行為沒問題,兩份實作才會慢慢不一致
