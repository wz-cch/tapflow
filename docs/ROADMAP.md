...
**編輯模式那一組留待之後討論** —— 它跟第 1、2 項的插入入口綁在一起。(前置的「暫停點可選取」已完成,所以這一組現在談得下去了。)

理由備註:

- **`⏸+` 不放待機** —— SPEC §10.1 定義非錄製狀態按它是「有選取就插在後面,沒有就插在尾端」,但**待機時不可能有選取**(`selectedStepId` 只在編輯模式會設),所以它永遠只能附加在尾端,弱到沒有存在價值。「插在選取後面」有價值,但那屬於編輯模式
- **`↩` 不放待機** —— SPEC §3.1 自己寫了它的存在理由:「從 M1 就要有⋯⋯**而完整的選取刪除排在 M2**」。M2 已經做完,它在待機的理由消失了。**前置條件(暫停點可選取)已完成**,所以現在拿掉是安全的 —— 在那之前拿掉會讓誤插的暫停點永遠刪不掉
...
1. ~~暫停點與等待成為可選取的標記~~ —— **已完成**,見 [CHANGELOG](../CHANGELOG.md)「暫停點刪不掉」。這是第 4 項的前置:沒有它,把 `↩` 從待機組拿掉會讓誤插的暫停點永遠刪不掉
2. **第 4 項**(視窗配置 + 按鍵組)—— 已定案,前置已完成,**可以動工**。順帶修掉真實的遮蔽問題與 `💾` 那個 bug
3. **第 2 項**(暫停點備註)—— 資料層已完成,複用 `WorkspaceDialogActivity` 的既有模式
4. **第 1 項**(插入等待)—— 與第 2 項共用輸入畫面。`WaitStep` 的標記已經畫好了,只差插入入口
5. **第 3 項**(指定起始步)—— 引擎便宜,語意已拍板
6. **第 5 項**(Shizuku)—— 大工程,等前面穩了再評估

2 → 3 → 4 之間只是效率考量(共用輸入畫面),可以調整。**編輯模式的按鍵組現在談得下去了** —— 暫停點能被選取之後,「插在選取的後面」才有意義。
